...
interface UndoDowngradeButtonProps {
  enabled: boolean;
  label: string;
}

export default function UndoDowngradeButton({ enabled, label }: UndoDowngradeButtonProps) {
  ...
  async function handleClick() {
    ...
    try {
      const response = await fetch('/api/stripe/keep-plan', {
        method: 'POST',
      });
      ...
      if (!response.ok) {
        throw new Error(payload.error || 'Could not keep the current plan.');
      }

      window.location.href = '/account?subscription=undo-downgrade';
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Could not keep the current plan.');
      setLoading(false);
    }
  }
